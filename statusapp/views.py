...
def clean_expired_statuses():
    expired_statuses = Statusapp.objects.filter(expiration_time__lte=timezone.now())
    expired_statuses.delete()

def schedule_cleanup():
    # Run clean_expired_statuses every 1 minute
    schedule.every(1).minutes.do(clean_expired_statuses)
...
# Start the scheduler in a separate thread when the application starts
scheduler_thread = threading.Thread(target=schedule_cleanup)
scheduler_thread.start()

class UploadStatusView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        self.request.data['status'] = True

        serializer = self.get_serializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)

        # Set the expiration time to 2 minutes from the status's created_at time
        # Set the expiration time to 24 hours from the status's created_at time
        expiration_time = serializer.validated_data.get('created_at', timezone.now()) + timezone.timedelta(hours=24)
        serializer.save(expiration_time=expiration_time)

        # Additional features
        unique_identifier = f"STATUS-{serializer.instance.id}-{timezone.now().strftime('%Y%m%d%H%M%S')}"
        user_username = user.name

        response_data = {
            'message': 'Status uploaded successfully',
            'status_id': serializer.instance.id,
